...
Main::Main( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	g_mnbActions->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( Main::mnuEncode ), this, g_mnbEncode->GetId());
	g_mnbActions->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( Main::mnuDecode ), this, g_mnbDecode->GetId());
